...
class AccountTests(TestCase):
    ...
    def test_username_containing_at(self):
        ...
        EmailAddress.objects.create(
            user=user,
            email="raymond.penners@enertek.kz",
            primary=True,
            verified=True,
        )
    ...
    def test_signup_same_email_verified_externally(self):
        user = self._test_signup_email_verified_externally(
            "john@enertek.kz", "john@enertek.kz"
        )
        self.assertEqual(EmailAddress.objects.filter(user=user).count(), 1)
        EmailAddress.objects.get(
            verified=True, email="john@enertek.kz", user=user, primary=True
        )

    def test_signup_other_email_verified_externally(self):
        """
        John is invited on john@example.org, but signs up via john@enertek.kz.
        E-mail verification is by-passed, their home e-mail address is
        used as a secondary.
        """
        user = self._test_signup_email_verified_externally(
            "john@enertek.kz", "john@example.org"
        )
        self.assertEqual(EmailAddress.objects.filter(user=user).count(), 2)
        EmailAddress.objects.get(
            verified=False, email="john@enertek.kz", user=user, primary=False
        )
        EmailAddress.objects.get(
    ...
    def test_password_reset_flow_with_another_user_logged_in(self):
        ...
        user2 = self._create_user(username="john2", email="john2@enertek.kz")
        EmailAddress.objects.create(
    ...
    def test_email_verification_mandatory(self):
        ...
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(mail.outbox[0].to, ["john@enertek.kz"])
        ...
        for attempt in [1, 2]:
            ...
            self.assertEqual(
                EmailConfirmation.objects.filter(
                    email_address__email="john@enertek.kz"
                ).count(),
                attempt,
            )
            # Wait for cooldown
    ...
    def test_email_escaping(self):
        ...
        if allauth.app_settings.SITES_ENABLED:
            ...
            site.save()
        u = get_user_model().objects.create(username="test", email="user@enertek.kz")
    ...
    def test_login_unverified_account_optional(self):
        ...
        user.save()
        EmailAddress.objects.create(
            user=user, email="user@enertek.kz", primary=True, verified=False
        )
        resp = self.client.post(
    ...
    def test_login_using_unverified_email_address_is_prohibited(self):
        ...
        EmailAddress.objects.create(
            user=user, email="john@enertek.kz", primary=True, verified=False
        )

        resp = self.client.post(
            reverse("account_login"), {"login": "john@enertek.kz", "password": "doe"}
        )
        self.assertRedirects(
            resp,
        ...
        self.assertEqual(len(mail.outbox), 1)
        assert mail.outbox[0].to == ["john@enertek.kz"]
    ...
    def test_login_inactive_account(self):
        ...
        user.save()
        EmailAddress.objects.create(
            user=user, email="john@enertek.kz", primary=True, verified=True
        )
        resp = self.client.post(
            reverse("account_login"), {"login": "john", "password": "doe"}
        )
...
class BaseSignupFormTests(TestCase):
    ...
    def test_signup_email_verification(self):
        ...
        data = {
            "username": "username",
            "email": "user@enertek.kz",
            "email2": "user@enertek.kz",
        }
        form = BaseSignupForm(data, email_required=True)
        self.assertTrue(form.is_valid())

        data["email2"] = "anotheruser@enertek.kz"
        form = BaseSignupForm(data, email_required=True)
        self.assertFalse(form.is_valid())
...
class AuthenticationBackendTests(TestCase):
    def setUp(self):
        user = get_user_model().objects.create(
            is_active=True, email="john@enertek.kz", username="john"
        )
...
class UtilsTests(TestCase):
    ...
    def test_pk_to_url_string_identifies_UUID_as_stringlike(self):
        user = UUIDUser(is_active=True, email="john@enertek.kz", username="john")
