...
class SocialAccountTests(TestCase):
    ...
    def test_email_address_created(self):
        ...
        setattr(user, account_settings.USER_MODEL_EMAIL_FIELD, "test@enertek.kz")
    ...
    def test_email_address_clash_username_required(self):
        ...
        request, resp = self._email_address_clash("test", "test@enertek.kz")
        self.assertEqual(resp["location"], reverse("socialaccount_signup"))

        # POST different username/email to social signup form
        request.method = "POST"
        request.POST = {"username": "other", "email": "other@enertek.kz"}
        resp = signup(request)
        self.assertEqual(resp["location"], "/accounts/profile/")
        user = get_user_model().objects.get(
            **{account_settings.USER_MODEL_EMAIL_FIELD: "other@enertek.kz"}
        )
    ...
    def test_email_address_clash_username_not_required(self):
        ...
        request.method = "POST"
        request.POST = {"email": "other@enertek.kz"}
        resp = signup(request)
        self.assertEqual(resp["location"], "/accounts/profile/")
        user = get_user_model().objects.get(
            **{account_settings.USER_MODEL_EMAIL_FIELD: "other@enertek.kz"}
    ...
    def test_email_address_clash_username_auto_signup(self):
        ...
        request, resp = self._email_address_clash("test", "other@enertek.kz")
        self.assertEqual(resp["location"], "/accounts/profile/")
        user = get_user_model().objects.get(
            **{account_settings.USER_MODEL_EMAIL_FIELD: "other@enertek.kz"}
        )
    ...
    def _email_address_clash(self, username, email):
        ...
        user_email(exi_user, "test@enertek.kz")
        exi_user.save()
    ...
    def test_disconnect(self):
        ...
        user_email(user, "test@enertek.kz")
        user.set_password("test")
        user.save()
    ...
    def test_verified_email_change_at_signup(self):
        ...
        sociallogin = SocialLogin(
            user=User(email="verified@enertek.kz"),
            account=SocialAccount(provider="google"),
            email_addresses=[
                EmailAddress(email="verified@enertek.kz", verified=True, primary=True)
            ],
        )
        ...
        self.assertEqual(form["email"].value(), "verified@enertek.kz")
        resp = self.client.post(
        ...
        self.assertEqual(user_email(user), "verified@enertek.kz")
        self.assertTrue(
            EmailAddress.objects.filter(
                user=user,
                email="verified@enertek.kz",
                verified=True,
                primary=True,
            ).exists()
        )
    ...
    def test_unverified_email_change_at_signup(self):
        ...
        User = get_user_model()
        sociallogin = SocialLogin(
            user=User(email="unverified@enertek.kz"),
            account=SocialAccount(provider="google"),
            email_addresses=[
                EmailAddress(
                    email="unverified@enertek.kz",
                    verified=False,
                    primary=True,
                )
            ],
        ...
        resp = self.client.get(reverse("socialaccount_signup"))
        form = resp.context["form"]
        self.assertEqual(form["email"].value(), "unverified@enertek.kz")
        resp = self.client.post(
            reverse("socialaccount_signup"),
            data={"email": "unverified@example.org"},
        )
    ...
    def test_unique_email_validation_signup(self):
        session = self.client.session
        User = get_user_model()
        User.objects.create(email="me@enertek.kz")
        sociallogin = SocialLogin(
            user=User(email="me@enertek.kz"),
            account=SocialAccount(provider="google"),
            email_addresses=[
                EmailAddress(email="me@enertek.kz", verified=True, primary=True)
            ],
        )
        session["socialaccount_sociallogin"] = sociallogin.serialize()
        session.save()
        resp = self.client.get(reverse("socialaccount_signup"))
        form = resp.context["form"]
        self.assertEqual(form["email"].value(), "me@enertek.kz")
        resp = self.client.post(
            reverse("socialaccount_signup"), data={"email": "me@enertek.kz"}
        )
    ...
    def test_social_account_taken_at_signup(self):
        ...
        User = get_user_model()
        sociallogin = SocialLogin(
            user=User(email="me1@enertek.kz"),
        ...
        resp = self.client.get(reverse("socialaccount_signup"))
        form = resp.context["form"]
        self.assertEqual(form["email"].value(), "me1@enertek.kz")
        resp = self.client.post(
            reverse("socialaccount_signup"),
            data={"username": "me1", "email": "me1@enertek.kz"},
        )
